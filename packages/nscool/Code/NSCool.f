...
c**** Input/Output:
     ...
     x              f_Heat,f_Bfield,f_Accretion,f_Strange
      dimension tprint(0:5000)  ! JK buffer size increased
...
C***** Local variable used only in the main program
     ...
     2          dephi(0:isize)
      ...
      dimension lambda_e(0:isize),lambda_mu(0:isize)
      dimension lambda_e1(0:isize),lambda_mu1(0:isize)
...
c ---------------------------------------------------------------------
     ...
     1  '**************************************************'
       print '(1a28,1a50)','****************************',
     1  '**************************************************'
!FIXME       read(5,*)        ! Wait for <ENTER> befor printing out
...
c *********************************************************************
     ...
     1              sig,lambda(i),lambda_e(i),lambda_mu(i),debug,
     2              nu_e_s,nu_e_l)
...
c *********************************************************************
     ...
     1      'Calling conduct',i,d,t
       call conduct(i,t,d,a,a1,z,qimp,nbfield2(i),
     1              sig,lambda1(i),lambda_e1(i),lambda_mu1(i),debug,
     2                   nu_e_s,nu_e_l)
...
!      write (*,*) itrial, ' ', ratiot, ' ', mratt  ! FIXME
!      write (*,*) '    ', ratiol, ' ', mratl  ! FIXME
!      write (*,*) '    ', ratios, ' ', mrats  ! FIXME
...
!      write (*,*) 'TIME STEP ', time, ' ', dtime ! FIXME
...
c ----------------------------------------------------------------------
      if (pscreen.eq.1) then
       if (i_acc.eq.0) then
         print '(i5,a4,1p1e12.3,3x,1p1e12.3,3x,1p3e12.3)',
     1        istep,'t=',(time+dtime)/year,teffective,
...
c *********************************************************************
      ...
      ! JK make sure time step is always significantly smaller than
      !    the output time step
!FIXME      if (itprint.gt.1) then
!       dtime=min(dtime, 0.1*(tprint(itprint)-tprint(itprint-1))*year)
!      end if
