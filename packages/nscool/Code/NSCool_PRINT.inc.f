...
514     format('# ',2(5x,1a7,1p1e10.3))
     ...
     3       'Temp [K]  ','Lum [Lsun]  ',
     4       'Q_mUrca  ','lambda_e  ','lambda_mu  ','Q_dUrca  ' !JK
516     format(
     1       a6,
     2       4a16,2x,
     3       2a16,2x,
     4       4a16,2x)                                  !JK
        write(10,'("# ",67x,2a16)')'no exp(phi) ','no exp(2phi) '
        write(10,'(a)')'#'
        do i=imax,1,-2
     ...
     4        qmurca_nucl(i) + qmurca_hyp(i) + qmurca_q(i),
     5        lambda_e(i),lambda_mu(i),
     6        qdurca_np(i) + qdurca_lap(i) + qdurca_smn(i)
     7           + qdurca_smla(i) + qdurca_sms0(i) + qdurca_q(i) !JK
        end do
        write(10,'(a)')'#'
517     format(
     1        i5,1x,
     ...
     4        1p4e16.7E3,2x)                  !JK
        ...
        ! JK: move to next printout time, taking into account the
        !     possibility that the timestep requested by the user
        !     is smaller than the simulation timestep
        do while ( ( ((time+dtime)/year) .ge. tprint(itprint) ) .and.
     1             (itprint.le.itpmax) )
         itprint=itprint+1
        end do
       end if
