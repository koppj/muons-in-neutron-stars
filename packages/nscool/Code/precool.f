...
c       INCLUDE 'profile_comp.inc.f'
       character*79 Model
       dimension rad_t(0:isize),bar_t(0:isize),rho_t(0:isize),
     1           pres_t(0:isize), emas_t(0:isize),phi_t(0:isize)
                              ! JK fixed to respect isize specification
...
c       INCLUDE 'control_nu.inc.f'
       INCLUDE 'quark.inc.f'
       dimension rho_t(0:isize),nbar_t(0:isize)
       dimension yneutr_t(0:isize),yprot_t(0:isize),
     2        yelect_t(0:isize),ymuon_t(0:isize),
     3        ylambda_t(0:isize),
     4        ysminus_t(0:isize),yszero_t(0:isize),ysplus_t(0:isize),
     5        yquarku_t(0:isize),yquarkd_t(0:isize),yquarks_t(0:isize),
     6        fhad_t(0:isize)
      dimension mstp_t(0:isize),mstn_t(0:isize),mstla_t(0:isize),
     2          mstsm_t(0:isize),msts0_t(0:isize),mstsp_t(0:isize)
      dimension theta_k_t(0:isize),theta_p_t(0:isize)
...
5768      i=i+1
          if((rrho(i0).ge.rho_t(i+1)).and.(rrho(i0).le.rho_t(i)))then
           ...
           goto 5768 ! FIXME this seems to happen too often
          end if
         end if
...
c       INCLUDE 'control_nu.inc.f'
       INCLUDE 'quark.inc.f'
       dimension rho_t(0:isize),nbar_t(0:isize),pres_t(0:isize)
       dimension yelect_t(0:isize),
     1       yquarku_t(0:isize),yquarkd_t(0:isize),yquarks_t(0:isize)
c *****
        if (debug.ge.1.) then
...
c ***** Read EOS file: *************************
         open(unit=59,file=f_stareos,status='old')
          ...
          do i=1,isize ! JK - changed upper limit from 1000 to isize
...
c       INCLUDE 'files_phys.inc.f'
       ...
       INCLUDE 'profile_star.inc.f'
       INCLUDE 'profile_comp.inc.f'
       dimension rho_t(0:isize),pres_t(0:isize),bar_t(0:isize),
     1           A_cell_t(0:isize),A_ion_t(0:isize),Z_ion_t(0:isize)
