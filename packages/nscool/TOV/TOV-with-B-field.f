     ...
     x          p(0:50000), em(0:50000), baryden(0:50000)
      ...
      common/prof/ r,den,e,p,em,phi,rhoc,baryden,propmas,delta,stepii
      common/Bfield/ Bs, B0, alpha, gamma, rho0
...
c This correspond to a density of 2.2e14 g/cm3 for the core limit
      rhod=2.573e-4   ! [1/fm^3]
c This correspond to a density of 4.3e11 g/cm3 for neutron drip
      const = 1.d0
      c2=1.d0
      target=1.44

      ! B field parameters according to eq. 13 of Watanabe Yanase Yoshinaga
      ! (https://academic.oup.com/ptep/article/2020/10/103E04/5900675#ptaa134M15)
      Bs    = 0.    ! [Gauss] - surface B field
      B0    = 0.    ! [Gauss] - B field at \rho -> \infty
      rho0  = 0.153 ! [1/fm^3] - nuclear saturation number density
      alpha = 0.05
      gamma = 2.
...
c      if(cread('standard')) stndrd = .true.
      ...
      if(cread('SMU')) then        ! base units are Msun/cm^3 for energy and pressure densities
        const = 1.0d0
        c2=1.0d0
      end if
      if(cread('MeV')) then
      ...
      if(cread('cgs')) then
        const = 1.0d0 / 1.989d18   ! conversion from grams/cm^3 to 1e-15 Msun/cm^3
        c2=9.d20                   ! 1/c2 is the conversion factor from cm/sec^2 to 1/cm
                                   ! -> const/c2 is the conversion factor from
                                   !    dyne/cm^2 to 1e-15 Msun/cm^3
      endif
     ...
     c                  rhol, 2.2e14*rhol/.1324
      ...
      if(dread('B0', B0, dum, 1, dum_l)) goto 5
      if(cread('B0?')) write(6,*)  B0
      if(dread('Bs', Bs, dum, 1, dum_l)) goto 5
      if(cread('Bs?')) write(6,*)  Bs
      goto 5
...
C ***** Do the next step **********************************************
      ...
      if (isnan(baryden(i+1))) then ! FIXME
        write (*,*) m1, m2, m3, m4
        write (*,*) step, bfunc
        stop -1
      end if
...
c *************************************************************************
      ...
      common/interp/ i1,i2,i3,limit
c      write(6,*) c2
C       bdir1="/Users/tobyopferkuch/Dropbox/CurrentProjects/"
C       bdir2="MuonicNSCooling/ns-muons/to/packages/nscool/EOS/"
C       title=bdir1//bdir2//title
      title='/Users/jkopp/cernbox/ns-muons/packages/nscool/EOS/'//title
...
c Check order of columns in the EOS file:
        ...
        B = Bs + B0 * (1 - exp( -alpha * (deos(i) / rho0)**gamma ))  !JK
!        write (*,*) peos(i), eeos(i), 0.5*B*B*5.611d-40 ! FIXME
!        write (*,*) "  ", 1 - exp( -alpha * (deos(i) / rho0)**gamma ) ! FIXME
        peos(i) = peos(i) + 0.5*B*B * 5.611d-40  ! JK convert Gauss^2 -> 1e-15 Msun/cm^3
        eeos(i) = eeos(i) + 0.5*B*B * 5.611d-40  ! JK
        if ( (i.gt.1).and.
     1       ((abs((peos(i)-peos(i-1))/(peos(i)+peos(i-1))).lt.1d-7).or.
     2        (abs((eeos(i)-eeos(i-1))/(eeos(i)+eeos(i-1))).lt.1d-7)) )
     3      then ! JK
          exit
        end if
       end do
...
c ************************************************************************
      ...
      ! incorporate B-field according to appendix A of Watanabe et al.,
      ! https://academic.oup.com/ptep/article/2020/10/103E04/5900675#ptaa134M15
      ! using the field proile from eq. 13 of that paper
!FIXME      B = Bs + B0 * (1 - exp( -alpha * (rho(p) / rho0)**gamma )) 
!FIXME      p = p + 0.5*B*B * 5.611d-40  ! JK convert Gauss^2 -> 1e-15 Msun/cm^3
      dens = rho(p)
      energy = ener(p)
!FIXME      energy = energy + 0.5*B*B * 5.611d-40  ! JK convert Gauss^2 -> 1e-15 Msun/cm^3
      vol = 4.*pi*r*r*r
     
      bfunc = dens *4.*pi*r*r / sqrt( 1.-2.*g*em / r )
      bfunc = bfunc * 8.42e-4
      write (*,*) dens, r, g*em/r ! FIXME
